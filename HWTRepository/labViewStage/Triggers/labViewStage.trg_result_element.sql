...
***********************************************************************************************************************************
	...
	Procedure:	hwt.trg_result_element
	Abstract:	Loads result_element records into staging environment

	Logic Summary
	-------------
	1)	Load trigger data into temp storage
	2)	Load repository result data from stage data
...
BEGIN TRY
	 ...
	 DECLARE	@CurrentID int ;
	  ...
	  SELECT	@CurrentID = ISNULL( MAX( ID ), 0 ) FROM labViewStage.result_element ;
...
--	2)	Load repository result data from stage data
	 EXECUTE	hwt.usp_LoadRepositoryFromStage
					@pSourceTable = N'result_element'
				;

--	3)	INSERT trigger data into labViewStage
	  INSERT	labViewStage.result_element
