...
--  3)  MERGE header test options from temp storage into hwt.HeaderOption
    ...
    WHEN MATCHED AND src.OptionValue <> tgt.OptionValue
        THEN UPDATE
            SET
                tgt.OptionValue =   src.OptionValue
              , tgt.UpdatedBy   =   src.UpdatedBy
              , tgt.UpdatedDate =   GETDATE()
    WHEN NOT MATCHED BY TARGET THEN
        INSERT(
            HeaderID, OptionID, OptionValue, UpdatedBy, UpdatedDate
        )
        VALUES(
            src.HeaderID, src.OptionID, src.OptionValue, src.UpdatedBy, GETDATE()
        )
    ;
