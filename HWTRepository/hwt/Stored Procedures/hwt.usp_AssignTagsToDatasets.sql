...
--	3)	load current assignments into temp storage
...
--	4)	Validation for too many of certain tag types
		WITH	newTags AS
				(
				  SELECT	TagID
						  , TagTypeName
		...
		BEGIN
			...
			GROUP BY	HeaderID, TagTypeName
			  ...
			  SELECT	@ErrorMessage	=	CASE
												...
												ELSE
													N'Cannot assign %1 tag %2 to dataset %5.  %1 tag %4 is already assigned.'
											END
